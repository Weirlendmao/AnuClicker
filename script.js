...
function showFloatingText(amount){
  ...
  el.innerText="+"+format(amount);
  container.appendChild(el);
  setTimeout(()=>el.remove(),1000);
}

// Hook into click
const oldClickObject = clickObject;
clickObject = function(){
  const before = score;
  oldClickObject();
  showFloatingText(score-before);
};
...
function exportSave(){
  ...
  a.download="idle_clicker_backup.json";
  a.click();
}
...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  document.getElementById("importFile")?.addEventListener("change",function(){
    if(this.files[0]) importSave(this.files[0]);
  });
});
